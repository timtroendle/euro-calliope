...
def area_weighted_time_series(shapes, spatiotemporal, gridcell_overlap_threshold):
    ...
    return pd.DataFrame(
        ...
        data={
            shape_id: weighted_time_series(weights_and_values.sel(shape_id=shape_id), gridcell_overlap_threshold)
...
def weights_between_shape_and_xy(shapes, stacked_spatiotemporal):
    ...
    grid_gdf = (
        gpd.GeoSeries(gpd.points_from_xy(x=x, y=y, crs=stacked_spatiotemporal.crs), crs=shapes.crs)
        .buffer(infer_resolution(stacked_spatiotemporal.unstack("xy")) / 2)
        .envelope
    )
    index_gdf = gpd.GeoDataFrame(geometry=grid_gdf, data={"xy": stacked_spatiotemporal.xy}, crs=shapes.crs)
...
def weighted_time_series(weights_and_values, gridcell_overlap_threshold):
    ds = (  # drop all locations with weight == 0 or value == np.nan
        weights_and_values.where(weights_and_values.weight > 0).dropna(subset=["weight", "value"], dim="xy", how="any")
    )
