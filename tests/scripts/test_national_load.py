...
class TestLoadHelperFunctions:
    ...
    @pytest.mark.parametrize(
        ...
        ([0, 2, [0, 1], [1, 3]], [0.5, 2, [np.nan, 1], [1, 3]], [0.75, 1.5, [np.nan, np.nan], [np.nan, 3]]),
    )
    def test_filter_outliers(self, min_thresh, max_tresh, expected_foo, expected_bar, foobar_df):
        ...
        config = {"outlier-data-thresholds": {"relative-to-mean-min": min_thresh, "relative-to-mean-max": max_tresh}}
    ...
    def test_interpolate_gaps(self, interpolate_timesteps, expected_foo, expected_bar, foobar_df):
        input_load = foobar_df(foo=[np.nan, 0, 1, 2, np.nan], bar=[0, np.nan, np.nan, np.nan, 4])
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            [
                2016,
                2017,
                ["2016-02-28 22:00:00", "2016-02-28 23:00:00"],
                ["2016-02-28 22:00:00", "2016-02-28 23:00:00"],
            ],
            [
                2016,
            ...
            [
                2016,
                2020,
                ["2016-02-28 23:00:00", "2016-02-29 00:00:00"],
                ["2016-02-28 23:00:00", "2016-02-29 00:00:00"],
            ],
            [
                2015,
    ...
    def test_ignore_feb_29th(self, model_year, next_available_year_of_data, input_timeseries, expected):
        ...
        filtered_timeseries = _ignore_feb_29th(model_year, next_available_year_of_data, input_timeseries)
    ...
    @pytest.mark.parametrize(
        ...
        (
            ...
            [
                2016,
                {
                    "2016-02-28 22:00:00": 1,
                    "2016-02-28 23:00:00": 2,
                    "2016-02-29 22:00:00": np.nan,
                    "2016-02-29 23:00:00": 3,
                },
                {
                    "2016-02-28 22:00:00": 1,
                    "2016-02-28 23:00:00": 2,
                    "2016-02-29 22:00:00": 1,
                    "2016-02-29 23:00:00": 3,
                },
            ],
        ),
    ...
    @pytest.mark.parametrize(
        ...
        ([[0, np.nan], [1, 3], ["foo"]], [[1, np.nan], [np.nan, 3], ["foo", "bar"]], [[0, 0], [1, 3], ["foo"]]),
    )
...
class TestLoadDummyData:
    ...
    def test_fail_on_too_low_interpolate(self, load, config_update, countries_in_err_msg, err_date):
