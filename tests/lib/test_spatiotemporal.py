...
def four_shapes():
    return gpd.GeoDataFrame(
        geometry=[box(0, 0, 2, 2), box(0, 2, 2, 4), box(2, 2, 4, 4), box(2, 0, 4, 2)], crs=EPSG3035
    )
...
def test_fails_without_crs_in_shapes(single_shape, any_spatiotemporal_data):
    ...
    with pytest.raises(AssertionError):
        area_weighted_time_series(
            shapes=single_shape, spatiotemporal=any_spatiotemporal_data, gridcell_overlap_threshold=DEFAULT_THRESHOLD
        )
...
@pytest.mark.parametrize("crs1,crs2", [(EPSG3035, EPSG27700), (WGS84, EPSG3035), (WGS84, EPSG27700)])
...
@pytest.mark.parametrize("correct_name,wrong_name", [("y", "lat"), ("x", "lon"), ("timestep", "timesteps")])
...
def test_equal_values(single_shape, make_spatiotemporal_data):
    ...
    weighted_ts = area_weighted_time_series(
        shapes=single_shape, spatiotemporal=spatiotemporal_data, gridcell_overlap_threshold=DEFAULT_THRESHOLD
    )
...
def test_four_shapes(four_shapes, make_spatiotemporal_data):
    ...
    weighted_ts = area_weighted_time_series(
        shapes=four_shapes, spatiotemporal=spatiotemporal_data, gridcell_overlap_threshold=DEFAULT_THRESHOLD
    )
...
def test_handles_not_too_many_nans_within_shapes(single_shape, make_spatiotemporal_data):
    spatiotemporal_data = make_spatiotemporal_data([0, np.nan, 2, 3])
    area_weighted_time_series(shapes=single_shape, spatiotemporal=spatiotemporal_data, gridcell_overlap_threshold=0.75)
...
def test_partial_match(single_shape_small, make_spatiotemporal_data):
    ...
    weighted_ts = area_weighted_time_series(
        shapes=single_shape_small, spatiotemporal=spatiotemporal_data, gridcell_overlap_threshold=DEFAULT_THRESHOLD
    )
...
def test_handles_reverted_coordinates_gracefully(single_shape, reverted_spatiotemporal_data):
    area_weighted_time_series(
        shapes=single_shape, spatiotemporal=reverted_spatiotemporal_data, gridcell_overlap_threshold=DEFAULT_THRESHOLD
    )
