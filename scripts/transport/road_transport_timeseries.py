...
def create_road_transport_demand_timeseries(
    ...
    if vehicle_type in ["passenger-cars", "motorcycles", "light-duty-vehicles"]:
        ...
        df_timeseries = df_annual.groupby(by=lambda idx: idx.year).transform(lambda x: x / len(x.index))
    ...
    assert not df_timeseries.isna().any(axis=None), "There are NaN values in the timeseries dataframe"
