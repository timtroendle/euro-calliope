...
def get_all_distance_efficiency(
    ...
    carrier_contribution = fill_missing_countries_and_years(
        road_energy.div(road_energy.groupby(level=["carrier", "country_code", "year"]).sum()), fill_missing_values
    )
    ...
    transport_distance_all_years = (
        transport_energy_per_mode.groupby(level=["country_code", "vehicle_subtype", "section", "vehicle_type", "year"])
        .sum()
        .mul(transport_efficiency)
    )

    # Use the distance traveled based on Eurostat to fill in blanks in JRC data
    aligned_dfs = transport_distance_all_years.reorder_levels(road_distance.index.names).align(road_distance)
...
def fill_missing_countries_and_years(jrc_data: pd.DataFrame, fill_missing_values: dict[str, str]) -> pd.DataFrame:
    ...
    with suppress(FileNotFoundError):  # it's fine. Just checking there is no MultiIndex in the columns
    ...
    jrc_data = jrc_data.assign(**{str(year): jrc_data[2015] for year in range(2016, 2019)})
...
if __name__ == "__main__":
    energy_balances = pd.read_csv(
        snakemake.input.energy_balances, index_col=["cat_code", "carrier_code", "unit", "country", "year"]
    ).squeeze()
    ...
    other_transportation_aviation = (  # all kerosene from the military destined for aviation
        energy_balances.loc[idx[FinalConsumption.OTHER_SECTORS, AVIATION_CARRIERS, :, :, :]]
    ...
    other_transport_road = (  # i.e. all oil that isn't destined for aviation
        energy_balances.loc[
            idx[
                [FinalConsumption.AGRICULTURE_AND_FORESTRY, FinalConsumption.OTHER_SECTORS],
    ...
    total_road_distance = total_road_distance.groupby(["vehicle_type", "country_code", "year"]).sum()

    # Extract historical electricity consumption
    total_historically_electrified_distance = (
        road_historically_electrified_consumption.groupby(level=["carrier", "vehicle_type", "country_code", "year"])
        .sum()
        .xs("electricity")
    )
