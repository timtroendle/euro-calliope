...
def process_jrc_transport_data(
    paths_to_data: list[str], dataset: object, out_path: str, vehicle_type_names: dict[str, str]
) -> None:
    paths_to_data = [Path(p) for p in paths_to_data]
    processed_data = pd.concat([read_transport_excel(path, **DATASET_PARAMS[dataset]) for path in paths_to_data])
...
def read_transport_excel(
    ...
    df = df.assign(country_code=column_names.split(" - ")[0]).set_index("country_code", append=True)
...
def process_road_vehicles(df: pd.DataFrame, column_names: str) -> pd.DataFrame:
    ...
    df = df.where((df.indent == 3) | (df.vehicle_type == "Powered 2-wheelers")).dropna(how="all")
    df.loc[df.vehicle_type == "Powered 2-wheelers", "vehicle_subtype"] = "Gasoline engine"
    return df.set_index(["section", "vehicle_type", "vehicle_subtype"]).drop([column_names, "indent"], axis=1)
