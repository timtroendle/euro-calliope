...
def create_national_timeseries(paths_to_input: list[str]) -> pd.DataFrame:
    all_ts = [pd.read_csv(path, index_col="utc-timestamp", parse_dates=True) for path in paths_to_input]
    return sum(all_ts)
...
def create_regional_timeseries(
    ...
    region_country_mapping = pd.read_csv(region_country_mapping, index_col=0).loc[:, "country_code"].to_dict()
    ...
    df_regional = (
        pd.DataFrame(
            ...
            data={id: df_national[country_code] for id, country_code in region_country_mapping.items()},
        )
