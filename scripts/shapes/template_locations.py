...
def construct_locations(path_to_shapes, path_to_template, resolution, path_to_output_yaml, path_to_output_csv):
    ...
    locations = locations.assign(centroid=locations.to_crs(EPSG3035).centroid.to_crs(WGS84).rename("centroid")).loc[
        :, ["name", "centroid"]
    ]

    locations = locations.assign(id=locations.index.str.replace(".", "-", regex=False)).set_index("id")

    parametrise_template(path_to_template, path_to_output_yaml, locations=locations, resolution=resolution)
