...
def _create_graph_with_land_connections(path_to_units):
    ...
    graph.add_nodes_from([
        (region_id, {"centroid": region_centroid}) for region_id, region_centroid in regions.centroid.to_dict().items()
    ])
    neighbours = {index: _neighbours(region.geometry, index, regions) for index, region in regions.iterrows()}
    graph.add_edges_from([(region, other) for region, all_others in neighbours.items() for other in all_others])
    return graph
