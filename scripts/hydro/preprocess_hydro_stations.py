...
def preprocess_stations(
    ...
    path_to_basins,
    buffer_size_m,
    path_to_phs_storage_capacities,
    scale_phs,
    path_to_output,
):
    ...
    stations.to_csv(path_to_output, header=True, index=True)
...
def estimate_missing_storage_capacity_values(stations):
    ...
    based_on_country_specific = (
        stations.merge(
            ...
            right_index=True,
            how="left",
        ).loc[:, "country_specific"]
        * stations.installed_capacity_MW
    )
    storage_capacity_MWh = stations.storage_capacity_MWh.where(pd.notnull, other=based_on_country_specific).where(
        pd.notnull, other=based_on_global
    )
...
def scale_phs_acording_to_geth(stations, path_to_geth_national_capacities):
    ...
    geth_national_capacities = read_national_phs_storage_capacities(path_to_geth_national_capacities).reindex(
        stations.loc[hphs_mask].country_code
    )
...
def ensure_stations_within_basins(stations, path_to_basins, buffer_size_m):
    geo_stations = gpd.GeoDataFrame(
        stations.copy(), geometry=gpd.points_from_xy(stations["lon"], stations["lat"]), crs=WGS84
    ).to_crs(EPSG3035)
...
if __name__ == "__main__":
    preprocess_stations(
        ...
        country_codes=[pycountry.countries.lookup(country).alpha_3 for country in snakemake.params.countries],
