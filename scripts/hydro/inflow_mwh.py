...
def determine_energy_inflow(
    path_to_stations_with_water_inflow, path_to_generation, first_year, final_year, max_capacity_factor, path_to_output
):
    plants_with_inflow_m3 = xr.open_dataset(path_to_stations_with_water_inflow)
    inflow_MWh = xr.merge([
        energy_inflow(
            plants_with_inflow_m3.sel(time=str(year)), read_generation(path_to_generation, year), max_capacity_factor
        )
...
def allocate_generation_to_plant(plants, annual_national_generation_mwh):
    ...
    capacity_share = (
        plants.assign(scaled_capacity=plants.installed_capacity_MW.multiply(inflow_count))
