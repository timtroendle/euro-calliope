...
def set_capacity_limits(
    ...
    capacities[
        ["eligibility_onshore_wind_monopoly_mw", "eligibility_onshore_wind_km2", "eligibility_onshore_wind_and_pv_km2"]
    ].to_csv(path_to_onshore_and_open_field_output)
    capacities[
        [
            "eligibility_offshore_wind_mw",
...
def _from_area_to_installed_capacity(land_eligibility_km2, roof_shares, maximum_installable_power_density):
    cap = land_eligibility_km2.copy()
    factor_rooftop = maximum_installable_power_density["pv-on-flat-areas"] * roof_shares[
        "flat"
    ...
    cap["eligibility_rooftop_pv_n_mw"] = (
        cap["eligibility_rooftop_pv_km2"] * roof_shares["N"] * maximum_installable_power_density["pv-on-tilted-roofs"]
    )
    ...
    cap["eligibility_rooftop_pv_s_flat_mw"] = (
        cap["eligibility_rooftop_pv_km2"] * roof_shares["S"] * maximum_installable_power_density["pv-on-tilted-roofs"]
    ) + (
        cap["eligibility_rooftop_pv_km2"] * roof_shares["flat"] * maximum_installable_power_density["pv-on-flat-areas"]
    )
    return cap
