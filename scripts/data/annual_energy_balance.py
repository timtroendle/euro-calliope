...
def generate_annual_energy_balance_nc(
    ...
    df = pd.read_csv(path_to_energy_balance, delimiter="\t", index_col=0, na_values=[":", ": ", ": z"])
    df.index = (
...
def get_ch_energy_balance_sheet(path_to_excel, sheet, skipfooter, cat_code):
    ...
    df.columns = (
        df.columns.get_level_values(0).str.translate(remove_digits).map(ch_energy_carriers).rename("carrier_code")
    )
...
def get_ch_waste_consumption(path_to_excel):
    ...
    waste_stream_gwh = pd.read_excel(
        path_to_excel, sheet_name=sheet_name, skiprows=5, index_col=0, header=[0, 1], skipfooter=8
...
def get_ch_transport_energy_balance(path_to_excel):
    ...
    def carrier_name_func(index):
        return df.columns.to_frame().iloc[:, index].str.translate(remove_digits).map(carriers)
...
def get_ch_industry_energy_balance(path_to_excel):
    ...
    column_names = (
        pd.read_excel(path_to_excel, sheet_name="Überblick_tot", skiprows=5, usecols="A,E:P", nrows=0, header=0)
