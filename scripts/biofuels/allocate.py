...
def biofuel_potential(
    ...
    national_potentials = (
        pd.read_csv(path_to_national_potentials, index_col=["year", "scenario", "country_code", "feedstock"])["value"]
        .mul(PJ_TO_MWH)
        .xs((potential_year, scenario), level=("year", "scenario"))
    )
    national_costs = (
        pd.read_csv(path_to_national_costs, index_col=["year", "scenario", "country_code", "feedstock"])["value"]
        .div(GJ_TO_MWH)
        .xs((potential_year, scenario), level=("year", "scenario"))
    )
    ...
    if (len(units.index) == 1) and (units.index[0] == "EUR"):  # special case for continental level
        cost = national_costs.mul(national_potentials).div(national_potentials.sum()).sum()
    ...
    weighted_cost = national_costs.mul(national_potentials).div(national_potentials.sum()).sum()
...
def allocate_potentials(national_potentials, units, population, land_cover, proxies):
    regional_potentials = pd.merge(
        national_potentials.reset_index(), units["country_code"].reset_index(), on="country_code"
