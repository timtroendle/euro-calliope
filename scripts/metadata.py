...
def metadata(config, version, path_to_output):
    metadata = {
        "description": "This is the metadata of the build process of "
    ...
    with open(path_to_output, "w", encoding="utf8") as outfile:
        yaml.dump(metadata, outfile, default_flow_style=False, allow_unicode=True, sort_keys=False)


if __name__ == "__main__":
    metadata(config=snakemake.params.config, version=snakemake.params.version, path_to_output=snakemake.output[0])
