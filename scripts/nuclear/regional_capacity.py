...
def regionalise_nuclear_capacity(
    ...
    capacity_current_points = [shapely.geometry.Point(xy) for xy in zip(capacity_current.lon, capacity_current.lat)]
    capacity_current_gdf = gpd.GeoDataFrame(capacity_current, geometry=capacity_current_points, crs="EPSG:4326")
    ...
    if nuclear_capacity_scenario == "current":
        capacity_per_region = capacity_current_per_region.to_frame("installed_capacity_nuclear_equals_MW")
    else:
        future_capacity = _get_future_capacity_from_config_file(nuclear_capacity_scenario, resolution, countries)
        ...
        nuclear_regional_proportion = capacity_current_per_region.groupby("country_code").transform(
            lambda x: x / x.sum()
        )

        capacity_per_region = future_capacity.mul(nuclear_regional_proportion, level="country_code", axis=0)
